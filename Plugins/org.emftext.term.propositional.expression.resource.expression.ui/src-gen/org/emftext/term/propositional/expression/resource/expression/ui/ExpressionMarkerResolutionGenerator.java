...
public class ExpressionMarkerResolutionGenerator implements org.eclipse.ui.IMarkerResolutionGenerator {
	...
	public org.eclipse.ui.IMarkerResolution[] getResolutions(org.eclipse.core.resources.IMarker marker) {
		try {
			...
			if (resource instanceof org.eclipse.core.resources.IFile) {
				...
				rs.getLoadOptions().put(org.emftext.term.propositional.expression.resource.expression.IExpressionOptions.DISABLE_CREATING_MARKERS_FOR_PROBLEMS, "true");
				org.eclipse.emf.ecore.resource.Resource emfResource = rs.getResource(uri, true);
				if (emfResource instanceof org.emftext.term.propositional.expression.resource.expression.mopp.ExpressionResource) {
					org.emftext.term.propositional.expression.resource.expression.mopp.ExpressionResource customResource = (org.emftext.term.propositional.expression.resource.expression.mopp.ExpressionResource) emfResource;
					org.eclipse.emf.ecore.util.EcoreUtil.resolveAll(customResource);
					java.util.Collection<org.emftext.term.propositional.expression.resource.expression.IExpressionQuickFix> quickFixes = getQuickFixes(customResource, marker);
					java.util.List<org.eclipse.ui.IMarkerResolution2> resolutions = new java.util.ArrayList<org.eclipse.ui.IMarkerResolution2>();
					for (final org.emftext.term.propositional.expression.resource.expression.IExpressionQuickFix quickFix : quickFixes) {
						resolutions.add(new org.eclipse.ui.IMarkerResolution2() {
							...
							public void run(org.eclipse.core.resources.IMarker marker) {
								...
								try {
									...
									org.emftext.term.propositional.expression.resource.expression.ui.ExpressionUIPlugin.logError("Exception while applying quick fix", e);
								}
							}
			...
			org.emftext.term.propositional.expression.resource.expression.ui.ExpressionUIPlugin.logError("Exception while computing quick fix resolutions", e);
		}
		return new org.eclipse.ui.IMarkerResolution[] {};
	}
	
	public java.util.Collection<org.emftext.term.propositional.expression.resource.expression.IExpressionQuickFix> getQuickFixes(org.emftext.term.propositional.expression.resource.expression.IExpressionTextResource resource, org.eclipse.core.resources.IMarker marker) {
		...
		try {
			...
			if (quickFixContexts != null) {
				...
				for (String quickFixContext : quickFixContextParts) {
					org.emftext.term.propositional.expression.resource.expression.IExpressionQuickFix quickFix = resource.getQuickFix(quickFixContext);
					if (quickFix != null) {
