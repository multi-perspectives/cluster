...
/**
 * The CslTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * CslTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class CslTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<org.feature.model.constraint.resource.csl.mopp.CslTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<org.feature.model.constraint.resource.csl.mopp.CslTaskItem> foundItems = new java.util.ArrayList<org.feature.model.constraint.resource.csl.mopp.CslTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new org.feature.model.constraint.resource.csl.mopp.CslTaskItem(keyword, message, localLine, offset, end));
