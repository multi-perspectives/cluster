...
public abstract class AbstractCslDebuggable {
	
	/**
	 * The list of breakpoints, where each breakpoint is represented by its location
	 * (a string) and the line number
	 */
	private java.util.List<org.feature.model.constraint.resource.csl.util.CslPair<String, Integer>> lineBreakpoints = new java.util.ArrayList<org.feature.model.constraint.resource.csl.util.CslPair<String, Integer>>();
	...
	public void startEventSocket(int eventPort) {
		try {
			...
			new org.feature.model.constraint.resource.csl.util.CslRuntimeUtil().logError("Can't create socket connection while launching.", e);
		}
	}
	...
	public void stopEventSocket() {
		try {
			...
			new org.feature.model.constraint.resource.csl.util.CslRuntimeUtil().logError("Exception while closing socket.", e);
		}
	}
	
	public void sendEvent(org.feature.model.constraint.resource.csl.debug.ECslDebugMessageTypes command, boolean sendOnlyInDebugMode, String... arguments) {
		if (isDebugMode() || !sendOnlyInDebugMode) {
			org.feature.model.constraint.resource.csl.debug.CslDebugMessage message = new org.feature.model.constraint.resource.csl.debug.CslDebugMessage(command, arguments);
	...
	public void evaluateLineBreakpoint(org.eclipse.emf.common.util.URI uri, int currentLine) {
		if (isDebugMode()) {
			...
			for (int i = 0; i < lineBreakpoints.size(); i++) {
				org.feature.model.constraint.resource.csl.util.CslPair<String, Integer> breakpointLocationAndLine = lineBreakpoints.get(i);
				...
				if (breakpointLine.intValue() == currentLine && breakpointLocation.equals(location)) {
					// suspending...
					setSuspend(true);
					sendEvent(org.feature.model.constraint.resource.csl.debug.ECslDebugMessageTypes.SUSPENDED, true, new String[] {"breakpoint", "" + currentLine});
					break;
				}
			}
			waitIfSuspended();
	...
	public void waitIfSuspended() {
		...
		sendEvent(org.feature.model.constraint.resource.csl.debug.ECslDebugMessageTypes.RESUMED, true);
	}
	
	public void addLineBreakpoint(String location, int line) {
		lineBreakpoints.add(new org.feature.model.constraint.resource.csl.util.CslPair<String, Integer>(location, new Integer(line)));
	}
	
	public void removeLineBreakpoint(String location, int line) {
		lineBreakpoints.remove(new org.feature.model.constraint.resource.csl.util.CslPair<String, Integer>(location, new Integer(line)));
	}
	...
	public void terminate() {
		sendEvent(org.feature.model.constraint.resource.csl.debug.ECslDebugMessageTypes.TERMINATED, false);
		stopEventSocket();
	}
	...
	public void startUpAndWait() {
		...
		if (isDebugMode()) {
			sendEvent(org.feature.model.constraint.resource.csl.debug.ECslDebugMessageTypes.STARTED, true);
			setSuspend(true);
