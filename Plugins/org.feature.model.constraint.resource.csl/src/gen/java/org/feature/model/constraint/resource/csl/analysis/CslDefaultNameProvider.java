...
public class CslDefaultNameProvider implements org.feature.model.constraint.resource.csl.ICslNameProvider {
		...
		if (nameAttr instanceof org.eclipse.emf.ecore.EAttribute) {
			...
			for (org.eclipse.emf.ecore.EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = org.feature.model.constraint.resource.csl.util.CslEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
