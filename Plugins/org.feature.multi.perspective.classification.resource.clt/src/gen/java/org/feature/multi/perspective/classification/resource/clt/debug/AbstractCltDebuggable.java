...
public abstract class AbstractCltDebuggable {
	
	/**
	 * The list of breakpoints, where each breakpoint is represented by its location
	 * (a string) and the line number
	 */
	private java.util.List<org.feature.multi.perspective.classification.resource.clt.util.CltPair<String, Integer>> lineBreakpoints = new java.util.ArrayList<org.feature.multi.perspective.classification.resource.clt.util.CltPair<String, Integer>>();
	...
	public void startEventSocket(int eventPort) {
		try {
			...
			new org.feature.multi.perspective.classification.resource.clt.util.CltRuntimeUtil().logError("Can't create socket connection while launching.", e);
		}
	}
	...
	public void stopEventSocket() {
		try {
			...
			new org.feature.multi.perspective.classification.resource.clt.util.CltRuntimeUtil().logError("Exception while closing socket.", e);
		}
	}
	
	public void sendEvent(org.feature.multi.perspective.classification.resource.clt.debug.ECltDebugMessageTypes command, boolean sendOnlyInDebugMode, String... arguments) {
		if (isDebugMode() || !sendOnlyInDebugMode) {
			org.feature.multi.perspective.classification.resource.clt.debug.CltDebugMessage message = new org.feature.multi.perspective.classification.resource.clt.debug.CltDebugMessage(command, arguments);
	...
	public void evaluateLineBreakpoint(org.eclipse.emf.common.util.URI uri, int currentLine) {
		if (isDebugMode()) {
			...
			for (int i = 0; i < lineBreakpoints.size(); i++) {
				org.feature.multi.perspective.classification.resource.clt.util.CltPair<String, Integer> breakpointLocationAndLine = lineBreakpoints.get(i);
				...
				if (breakpointLine.intValue() == currentLine && breakpointLocation.equals(location)) {
					// suspending...
					setSuspend(true);
					sendEvent(org.feature.multi.perspective.classification.resource.clt.debug.ECltDebugMessageTypes.SUSPENDED, true, new String[] {"breakpoint", "" + currentLine});
					break;
				}
			}
			waitIfSuspended();
	...
	public void waitIfSuspended() {
		...
		sendEvent(org.feature.multi.perspective.classification.resource.clt.debug.ECltDebugMessageTypes.RESUMED, true);
	}
	
	public void addLineBreakpoint(String location, int line) {
		lineBreakpoints.add(new org.feature.multi.perspective.classification.resource.clt.util.CltPair<String, Integer>(location, new Integer(line)));
	}
	
	public void removeLineBreakpoint(String location, int line) {
		lineBreakpoints.remove(new org.feature.multi.perspective.classification.resource.clt.util.CltPair<String, Integer>(location, new Integer(line)));
	}
	...
	public void terminate() {
		sendEvent(org.feature.multi.perspective.classification.resource.clt.debug.ECltDebugMessageTypes.TERMINATED, false);
		stopEventSocket();
	}
	...
	public void startUpAndWait() {
		...
		if (isDebugMode()) {
			sendEvent(org.feature.multi.perspective.classification.resource.clt.debug.ECltDebugMessageTypes.STARTED, true);
			setSuspend(true);
