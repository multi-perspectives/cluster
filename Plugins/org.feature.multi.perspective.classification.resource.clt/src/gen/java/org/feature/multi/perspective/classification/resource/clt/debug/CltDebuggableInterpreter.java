 ...
 */
public class CltDebuggableInterpreter<ResultType, ContextType> extends org.feature.multi.perspective.classification.resource.clt.debug.AbstractCltDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private org.feature.multi.perspective.classification.resource.clt.util.AbstractCltInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private org.feature.multi.perspective.classification.resource.clt.ICltCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public CltDebuggableInterpreter(org.feature.multi.perspective.classification.resource.clt.util.AbstractCltInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new org.feature.multi.perspective.classification.resource.clt.ICltInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = org.feature.multi.perspective.classification.resource.clt.util.CltStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public org.feature.multi.perspective.classification.resource.clt.util.AbstractCltInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		org.feature.multi.perspective.classification.resource.clt.ICltLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private org.feature.multi.perspective.classification.resource.clt.ICltLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		org.feature.multi.perspective.classification.resource.clt.ICltCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(org.feature.multi.perspective.classification.resource.clt.debug.ECltDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = org.feature.multi.perspective.classification.resource.clt.util.CltEObjectUtil.getDepth(current);
		stopCondition = new org.feature.multi.perspective.classification.resource.clt.ICltCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = org.feature.multi.perspective.classification.resource.clt.util.CltEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = org.feature.multi.perspective.classification.resource.clt.util.CltEObjectUtil.getDepth(current) - 1;
		stopCondition = new org.feature.multi.perspective.classification.resource.clt.ICltCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
