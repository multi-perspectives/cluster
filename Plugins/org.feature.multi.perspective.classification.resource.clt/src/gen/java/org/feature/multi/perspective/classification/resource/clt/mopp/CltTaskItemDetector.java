...
/**
 * The CltTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * CltTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class CltTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<org.feature.multi.perspective.classification.resource.clt.mopp.CltTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<org.feature.multi.perspective.classification.resource.clt.mopp.CltTaskItem> foundItems = new java.util.ArrayList<org.feature.multi.perspective.classification.resource.clt.mopp.CltTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new org.feature.multi.perspective.classification.resource.clt.mopp.CltTaskItem(keyword, message, localLine, offset, end));
