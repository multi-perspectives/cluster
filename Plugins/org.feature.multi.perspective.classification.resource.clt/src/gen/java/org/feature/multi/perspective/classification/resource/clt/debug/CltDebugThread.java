...
public class CltDebugThread extends org.feature.multi.perspective.classification.resource.clt.debug.CltDebugElement implements org.eclipse.debug.core.model.IThread, org.feature.multi.perspective.classification.resource.clt.debug.ICltDebugEventListener {
	
	private boolean suspended = false;
	private org.feature.multi.perspective.classification.resource.clt.debug.CltDebugTarget debugTarget;
	
	public CltDebugThread(org.feature.multi.perspective.classification.resource.clt.debug.CltDebugTarget target) {
		super(target);
	...
	public org.eclipse.debug.core.model.IStackFrame[] getStackFrames() throws org.eclipse.debug.core.DebugException {
		if (isSuspended()) {
			org.feature.multi.perspective.classification.resource.clt.debug.CltDebugMessage stack = this.debugTarget.getDebugProxy().getStack();
			String framesData = stack.getArgument(0);
			if (framesData != null && !"".equals(framesData)) {
				java.util.List<String> frames = org.feature.multi.perspective.classification.resource.clt.util.CltStringUtil.decode(framesData, '#');
				...
				for (int i = 0; i < frames.size(); i++) {
					...
					theFrames[frames.size() - i - 1] = new org.feature.multi.perspective.classification.resource.clt.debug.CltStackFrame(getTarget(), data);
				}
				return theFrames;
			}
		}
	...
	public org.feature.multi.perspective.classification.resource.clt.debug.CltDebugTarget getTarget() {
		return debugTarget;
	}
