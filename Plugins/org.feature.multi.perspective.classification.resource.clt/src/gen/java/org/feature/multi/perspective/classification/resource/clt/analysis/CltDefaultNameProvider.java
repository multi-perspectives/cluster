...
public class CltDefaultNameProvider implements org.feature.multi.perspective.classification.resource.clt.ICltNameProvider {
		...
		if (nameAttr instanceof org.eclipse.emf.ecore.EAttribute) {
			...
			for (org.eclipse.emf.ecore.EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = org.feature.multi.perspective.classification.resource.clt.util.CltEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
