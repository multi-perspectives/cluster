...
public class ClassificationGenerator extends AbstractGenerator {
	...
	public ClassificationGenerator() {
		super();
		classifyModel = ClassificationFactoryImpl.eINSTANCE
	...
	public void generateClassification() {
		// TODO: need to get the mapping model somehow
		log.error("Not yt for plugin implemented");
	}

	public void generateClassification(MappingModel featuremapping) {
		FeatureModel fm = featuremapping.getFeatureModel();
		GroupModel gm = featuremapping.getViewModel();
		...
		for (Mapping m : featuremapping.getMappings()) {
			...
			if (g instanceof CoreGroup)
				continue;

			EList<Feature> viewFeatures = m.getFeatures();

			// TODO: sind in einem mapping auch automatisch alle parent-mappings
			// enthalten? Oder nur das delta zum parent?

			for (int i = 0; i < numberOfParalellViewInterpretationScenarios; i++) {

				Classification c = new ClassificationFactoryImpl().eINSTANCE
						.createClassification();

				c.setViewgroup(g);
				c.setId(g.getName() + "_" + i);

				for (Feature f : viewFeatures) {
					int randomUnbound = (int) Math
							.floor((Math.random() * 99) + 1);
					int randomUnclassified = (int) Math
							.floor((Math.random() * 99) + 1);
					...
					// if both would match, unclassified wins
						...
						if (productVariants[i].getAliveFeatures().contains(f))
							c.getAliveFeatures().add(f);
						else if (productVariants[i].getDeadFeatures().contains(
								f))
							c.getDeadFeatures().add(f);
						else
							log.error("Feature classification is UNKNOWN! - Classification cannot be determined, set to unclassified");
					}
				}
				classifyModel.getClassifications().add(c);
			}
		}
	}
	...
	private void fillViewInterpretations(FeatureModel fm) {
		...
		for (int i = 0; i < numberOfParalellViewInterpretationScenarios; i++) {
			...
			FullVariantClassification variantClassification = new FullVariantClassification(
					variant);
			...
			log.debug("Adding Variant-Interpretion " + i + " with "
					+ variantClassification.getAliveFeatures().size()
					+ " alive features and "
					+ variantClassification.getDeadFeatures().size()
					+ " dead features");
		}
