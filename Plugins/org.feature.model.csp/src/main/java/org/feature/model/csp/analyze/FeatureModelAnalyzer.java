...
public class FeatureModelAnalyzer {
	...
	public List<Constraint> getAllCSPConstraints() {
		if (allCSPConstraints == null) {
			allCSPConstraints = FeatureModelUtil.getConstraints(model,
					FeatureModelUtil.csp_constraintLanguage);
		}
		return allCSPConstraints;
	}

	public int getConstraintCoverage() {
		Set<Feature> consFeatures = TextExpressionParser
				.getConstrainedFeatures(model);
	...
	private void determineCoreAndVariableFeatures() {
		...
		for (Feature feature : features) {
			...
			while (isCommon && iterator.hasNext()) {
				FeatureVariant featureVariant = (FeatureVariant) iterator
						.next();
	...
	public List<FeatureVariant> getAllVariants() {

		LinkedList<FeatureVariant> listOfProducts = new LinkedList<FeatureVariant>();
		...
		// solver.solveAll();
		// int numConfigs = solver.getNbSolutions();

		if (solver.isFeasible()) {
			int variantNumber = 1;
			// parse all solutions
			do {
				DisposableIterator<IntDomainVar> variables = solver
						.getIntVarIterator();
				...
				while (variables.hasNext()) {
					...
					Feature feature = getFeature(variable.getName());
					int cardinality = variable.getVal();

					if (variable != null && cardinality > 0 && feature != null) {
						variant.addFeature(feature);
					}
					log.debug(variable.getName() + ":" + variable.getVal()
							+ " - added to Variant number " + variantNumber);
				}

				if (variant.getFeatureCount() > 0
						&& !isVariantInVariantSet(variant, listOfProducts)) {
					listOfProducts.add(variant);
					variantNumber++;
				}

				// jump to next solver-solution
	...
	private boolean isVariantInVariantSet(FeatureVariant variant,
			List<FeatureVariant> listVariants) {

		for (FeatureVariant v : listVariants) {
			if (v.getFeatureCount() == variant.getFeatureCount())
				if (v.getFeatures().containsAll(variant.getFeatures())
						&& variant.getFeatures().containsAll(v.getFeatures()))
					return true;
				else
					continue;
		}
	...
	private void addCardinality(Feature feature, int cardinality) {
		FeatureModelUtil.setAttribute(feature,
				FeatureModelUtil.attribute_cardinality,
