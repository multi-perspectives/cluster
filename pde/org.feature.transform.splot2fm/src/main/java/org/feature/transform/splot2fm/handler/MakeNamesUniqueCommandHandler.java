...
public class MakeNamesUniqueCommandHandler extends AbstractGenCommandHandler {
   ...
   public Object execute(ExecutionEvent event) throws ExecutionException {
      ...
      for (IFile iFile : files) {
         FeatureModel featureModel = FeatureModelUtil.getFeatureModel(iFile, resourceSet);
         if (featureModel != null) {
            boolean isChanged = FeatureModelUtil.makeFeatureNamesUnique(featureModel);
            if (isChanged) {
               FeatureModelUtil.persistFMFeatureModel(featureModel, iFile.getRawLocation().toOSString());
            } else {
