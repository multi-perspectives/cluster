...
public class ExpressionOccurrence {
	...
	public final static String OCCURRENCE_ANNOTATION_ID = "org.emftext.term.propositional.expression.resource.expression.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "org.emftext.term.propositional.expression.resource.expression.ui.occurences.declaration";
	...
	private org.emftext.term.propositional.expression.resource.expression.ui.ExpressionTokenScanner tokenScanner;
	 ...
	 */
	public ExpressionOccurrence(org.emftext.term.propositional.expression.resource.expression.IExpressionTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, org.emftext.term.propositional.expression.resource.expression.ui.ExpressionTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new org.emftext.term.propositional.expression.resource.expression.mopp.ExpressionMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(org.emftext.term.propositional.expression.resource.expression.ui.ExpressionBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, org.emftext.term.propositional.expression.resource.expression.ui.ExpressionPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addAnnotation(document, org.emftext.term.propositional.expression.resource.expression.ui.ExpressionPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, org.emftext.term.propositional.expression.resource.expression.ui.ExpressionPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == org.emftext.term.propositional.expression.resource.expression.ui.ExpressionPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(org.emftext.term.propositional.expression.resource.expression.ui.ExpressionOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(org.emftext.term.propositional.expression.resource.expression.ui.ExpressionOccurrence.DECLARATION_ANNOTATION_ID);
	}
