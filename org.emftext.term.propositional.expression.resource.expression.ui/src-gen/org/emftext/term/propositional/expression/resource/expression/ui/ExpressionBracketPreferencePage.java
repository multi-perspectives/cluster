 ...
 * </ul>
 */
public class ExpressionBracketPreferencePage extends org.eclipse.jface.preference.PreferencePage implements org.eclipse.ui.IWorkbenchPreferencePage {
	
	private static final String[] ALL_LEFT_BRACKETS = new String[] { "{", "(", "[", "<", "\"", "'", };
	private static final String[] ALL_RIGHT_BRACKETS = new String[] { "}", ")", "]", ">", "\"", "'", };
	
	private String BRACKETS_COLOR = org.emftext.term.propositional.expression.resource.expression.ui.ExpressionPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR;
	...
	private String language = new org.emftext.term.propositional.expression.resource.expression.mopp.ExpressionMetaInformation().getSyntaxName();
	
	private org.emftext.term.propositional.expression.resource.expression.ui.ExpressionBracketSet bracketsTmp;
	
	/**
	 * Creates a preference page for bracket setting.
	 */
	public ExpressionBracketPreferencePage() {
		super();
		
		org.emftext.term.propositional.expression.resource.expression.IExpressionMetaInformation metaInformation = new org.emftext.term.propositional.expression.resource.expression.mopp.ExpressionMetaInformation();
	...
	public void init(org.eclipse.ui.IWorkbench workbench) {
		setPreferenceStore(org.emftext.term.propositional.expression.resource.expression.ui.ExpressionUIPlugin.getDefault().getPreferenceStore());
		setDescription("Define the coloring of matching brackets.");
		
		bracketsTmp = new org.emftext.term.propositional.expression.resource.expression.ui.ExpressionBracketSet(null, null);
		for (String languageID : languageIDs) {
			bracketSetTemp.put(languageID, getPreferenceStore().getString(languageID + org.emftext.term.propositional.expression.resource.expression.ui.ExpressionPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		}
	}
	...
	private void handleMatchingBracketsSelection() {
		...
		enableCheckbox.setSelection(getPreferenceStore().getBoolean(		org.emftext.term.propositional.expression.resource.expression.ui.ExpressionPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		enableClosingInside.setSelection(false);
		matchingBracketsColorButton.setEnabled(getPreferenceStore().getBoolean(		org.emftext.term.propositional.expression.resource.expression.ui.ExpressionPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		...
		initializeLanguage();
		bracketsTmp.setBrackets(getPreferenceStore().getString(language + org.emftext.term.propositional.expression.resource.expression.ui.ExpressionPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		...
		bracketsList.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int index : itemIndices) {
					org.emftext.term.propositional.expression.resource.expression.IExpressionBracketPair bracketPair = bracketsTmp.getBracketPair(index);
		...
		enableClosingInside.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int idx : itemIndices) {
					org.emftext.term.propositional.expression.resource.expression.IExpressionBracketPair bracketPair = bracketsTmp.getBracketPair(idx);
	...
	protected void performDefaults() {
		enableCheckbox.setSelection(getPreferenceStore().getDefaultBoolean(org.emftext.term.propositional.expression.resource.expression.ui.ExpressionPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		matchingBracketsColorButton.setEnabled(enableCheckbox.getSelection());
		...
		bracketSetTemp.put(language, getPreferenceStore().getDefaultString(language + org.emftext.term.propositional.expression.resource.expression.ui.ExpressionPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		bracketsTmp.setBrackets(bracketSetTemp.get(language));
		bracketsList.setItems(bracketsTmp.getBracketArray());
		enableClosingInside.setSelection(false);
	}
	...
	private void updateActiveEditor() {
		...
		getPreferenceStore().setValue(org.emftext.term.propositional.expression.resource.expression.ui.ExpressionPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX, enableCheckbox.getSelection());
		getPreferenceStore().setValue(language + org.emftext.term.propositional.expression.resource.expression.ui.ExpressionPreferenceConstants.EDITOR_BRACKETS_SUFFIX, bracketSetTemp.get(language));
		...
		if (editor != null && editor instanceof org.emftext.term.propositional.expression.resource.expression.ui.ExpressionEditor) {
			((org.emftext.term.propositional.expression.resource.expression.ui.ExpressionEditor) editor).invalidateTextRepresentation();
		}
	}
